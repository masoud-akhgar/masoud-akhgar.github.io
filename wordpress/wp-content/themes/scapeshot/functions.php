 ...
 */
function scapeshot_mejs_add_container_class() {
	if ( ! wp_script_is( 'mediaelement', 'done' ) ) {
		return;
	}

	$next_track_text   = esc_attr__( 'Next Track', 'scapeshot' );
	$prev_track_text   = esc_attr__( 'Previous Track', 'scapeshot' );
	$toggle_text       = esc_attr__( 'Toggle Playlist', 'scapeshot' );
...
add_action( 'wp_print_footer_scripts', 'scapeshot_mejs_add_container_class' );
...
if ( ! function_exists( 'scapeshot_excerpt_length' ) ) :
	...
	function scapeshot_excerpt_length( $length ) {
		...
		$length	= get_theme_mod( 'scapeshot_excerpt_length', 30 );
